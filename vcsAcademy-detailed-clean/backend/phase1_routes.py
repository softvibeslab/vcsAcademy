...
BADGES = [
    {
        "badge_id": "badge_foundation",
        "name": "Foundation Builder",
        "description": "Completed Track 1: Pro Mindset",
        "criteria": "Complete all modules in Track 1",
        "criteria_type": "track_complete",
        "criteria_value": "track_1",
        "icon": "foundation",
        "order": 1,
    },
    {
        "badge_id": "badge_discovery",
        "name": "Discovery Master",
        "description": "Completed Track 2: Discovery & Control",
        "criteria": "Complete all modules in Track 2",
        "criteria_type": "track_complete",
        "criteria_value": "track_2",
        "icon": "search",
        "order": 2,
    },
    {
        ...
        "description": "Completed Track 3: Value Architecture",
        "criteria": "Complete all modules in Track 3",
        "criteria_type": "track_complete",
        "criteria_value": "track_3",
        "icon": "diamond",
        "order": 3,
    },
    {
        "badge_id": "badge_decision",
        "name": "Decision Controller",
        "description": "Completed Track 4: Decision Management",
        "criteria": "Complete all modules in Track 4",
        "criteria_type": "track_complete",
        "criteria_value": "track_4",
        "icon": "target",
        "order": 4,
    },
    {
        "badge_id": "badge_objection",
        "name": "Objection Handler",
        "description": "Completed Track 5: Objection Mastery",
        "criteria": "Complete all modules in Track 5",
        "criteria_type": "track_complete",
        "criteria_value": "track_5",
        "icon": "shield",
        "order": 5,
    },
    {
        "badge_id": "badge_integrity",
        "name": "Integrity Closer",
        "description": "Completed Track 6: Post-Decision Integrity",
        "criteria": "Complete all modules in Track 6",
        "criteria_type": "track_complete",
        "criteria_value": "track_6",
        "icon": "lock",
        "order": 6,
    },
    {
        "badge_id": "badge_analyst",
        "name": "Deal Analyst",
        "description": "Reviewed 10 Deal Breakdowns",
        "criteria": "Review 10 deal breakdowns",
        "criteria_type": "content_count",
        "criteria_value": "deal_breakdown:10",
        "icon": "magnifier",
        "order": 7,
    },
    {
        "badge_id": "badge_tactician",
        "name": "Sales Tactician",
        "description": "Applied 15 Quick Wins",
        "criteria": "Apply 15 Quick Wins",
        "criteria_type": "content_count",
        "criteria_value": "quick_win:15",
        "icon": "zap",
        "order": 8,
    },
    {
        "badge_id": "badge_consistent",
        "name": "Consistent Learner",
        "description": "14-day training streak",
        "criteria": "Maintain a 14-day training streak",
    ...
    {
        "badge_id": "badge_topproducer",
        "name": "Top Producer",
        "description": "Completed all tracks and reached elite status",
        "criteria": "Complete all 6 tracks",
        "criteria_type": "stage",
        "criteria_value": "4",
        "icon": "trophy",
        "order": 11,
...
async def get_track_modules(track_id: str) -> List[Dict]:
    ...
    if not track_content:
        try:
            db_track_content = await db.phase1_track_content.find(
                {"track_id": track_id}
            ).to_list(50)
            if db_track_content:
    ...
    for tc in sorted(track_content, key=lambda x: x.get("order_index", 0)):
        ...
        if not content:
            try:
                db_content = await db.phase1_content.find_one(
...
def calculate_readiness_score(progress: Dict) -> int:
    ...
    qw_completed = len(
        [c for c in progress.get("content_completed", []) if c.startswith("qw_")]
    )
    qw_score = min(qw_completed / 20, 1)

    # Deal breakdowns (out of 15)
    bd_completed = len(
        [c for c in progress.get("content_completed", []) if c.startswith("breakdown_")]
    )
    ...
    if criteria_type == "track_complete":
        ...
        if content_type == "deal_breakdown":
            completed = len(
                [
                    c
                    for c in progress.get("content_completed", [])
                    if c.startswith("breakdown_")
                ]
            )
...
async def get_tracks():
    ...
    for track in TRACKS:
        ...
        tracks_with_modules.append(
            {**track, "modules": modules, "module_count": len(modules)}
        )
...
async def get_track(track_id: str):
    ...
    return {**track, "modules": modules, "module_count": len(modules)}
...
async def get_breakdown(breakdown_id: str, user: User = Depends(require_auth)):
    ...
    breakdown = next(
        (b for b in DEAL_BREAKDOWNS if b["content_id"] == breakdown_id), None
    )
    if not breakdown:
...
async def get_masterclasses(
    ...
    if expert:
        all_content = [
            ...
            if expert.lower() in c.get("expert", {}).get("name", "").lower()
        ]
    ...
    if sort == "popular":
        ...
        all_content.sort(
            key=lambda x: x.get("recorded_date", x.get("created_at", datetime.min)),
            reverse=True,
        )
...
async def get_user_progress(user: User = Depends(require_auth)):
    ...
    current_stage = next(
        (s for s in STAGES if s["stage_number"] == progress.get("current_stage", 1)),
        STAGES[0],
    )
    next_stage = next(
        (
            s
            for s in STAGES
            if s["stage_number"] == progress.get("current_stage", 1) + 1
        ),
        None,
    )
    ...
    completed_modules = len(
        [c for c in progress.get("content_completed", []) if c.startswith("mod_")]
    )
    completed_breakdowns = len(
        [c for c in progress.get("content_completed", []) if c.startswith("breakdown_")]
    )
    completed_quickwins = len(
        [c for c in progress.get("content_completed", []) if c.startswith("qw_")]
    )
...
async def complete_content(content_id: str, user: User = Depends(require_auth)):
    ...
    if content_id not in progress.get("content_completed", []):
        progress["content_completed"] = progress.get("content_completed", []) + [
            content_id
        ]
        ...
        if content_id.startswith("mod_"):
            ...
            track_mapping = next(
                (tc for tc in TRACK_CONTENT if tc["content_id"] == content_id), None
            )
            if track_mapping:
                track_id = track_mapping["track_id"]
                track_modules = await get_track_modules(track_id)
                completed_in_track = len(
                    [
                        m
                        for m in track_modules
                        if m["content_id"] in progress["content_completed"]
                    ]
                )
                track_progress = (
                    (completed_in_track / len(track_modules)) * 100
                    if track_modules
                    else 0
                )
        ...
        # Add training time
        progress["total_training_time"] = progress.get(
            "total_training_time", 0
        ...
        if points_earned > 0:
            await db.users.update_one(
                {"user_id": user.user_id}, {"$inc": {"points": points_earned}}
            )

        # Save progress
        await db.user_progress.update_one(
            {"user_id": user.user_id}, {"$set": progress}, upsert=True
        )
    ...
    if current_stage < 4:
        next_stage_def = STAGES[
            current_stage
        ]  # 0-indexed, so current_stage gets the next one
...
async def get_bookmarks(tag: Optional[str] = None, user: User = Depends(require_auth)):
    ...
    for bm in bookmarks:
        ...
        if content:
            enriched.append({**bm, "content": content})
...
async def create_bookmark(request: Request, user: User = Depends(require_auth)):
    ...
    existing = await db.bookmarks.find_one(
        {"user_id": user.user_id, "content_id": content_id}, {"_id": 0}
    )
    if existing:
...
async def get_before_tour_content(user: User = Depends(require_auth)):
    ...
    bookmarks = await db.bookmarks.find(
        {"user_id": user.user_id, "tag": "before_tour"}, {"_id": 0}
    ).to_list(20)
    ...
    suggested_quickwins = [q for q in QUICK_WINS if "before_tour" in q.get("tags", [])][
        :5
    ]
    suggested_breakdowns = [
        b for b in DEAL_BREAKDOWNS if "before_tour" in b.get("tags", [])
    ][:3]
    ...
    return {
        ...
        "total_duration": sum(
            c.get("duration", 0) for c in suggested_modules + suggested_quickwins
        ),
    }
...
async def review_breakdown_legacy(
    breakdown_id: str, user: User = Depends(require_auth)
):
    """Legacy endpoint - redirects to new content completion"""
