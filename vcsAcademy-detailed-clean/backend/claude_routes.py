"""
Ollama AI Assistant Routes
API endpoints for the intelligent sales assistant powered by Ollama (Local LLM)
"""
...
def build_system_prompt(user_context: Dict[str, Any]) -> str:
    """Build optimized system prompt for Ollama with user context"""
    ...
    # More concise prompt for Ollama
    prompt = f"""You are VCSA Coach, a sales assistant for vacation club professionals.

USER: {user['name']} | Level: {user['level']} | Points: {user['points']}

FINANCIAL GOAL ({dates['current_month']}):
🎯 Target: ${financial['target_income']:,.0f}
💰 Gap: ${financial['income_gap']:,.0f}
📊 Need: {financial['sales_needed']} sales, {financial['tours_needed']} tours
📈 Avg Sale: ${financial['avg_sale']:,.0f} | Close Rate: {financial['closing_rate']}%

PERFORMANCE:
✅ Sales: {sales['sales_count']} | Volume: ${sales['total_volume']:,.0f}
💪 Today: {today['attributes_achieved']}/7 attributes | {today['challenges_completed']}/{today['challenges_total']} challenges
⏰ {dates['days_remaining']} days left
...
"""
    ...
    if sales['recent_sales']:
        for sale in sales['recent_sales']:
            prompt += f"Day {sale['day']}: ${sale['volume']:,.0f}"
            if sale.get('customer'):
                prompt += f" - {sale['customer']}"
            prompt += "\n"
    else:
        prompt += "No sales yet\n"
...
- Track goals and give specific recommendations
- Motivate with data-driven insights
- Suggest training modules
- Remind about 7 personal attributes

STYLE:
- Professional but friendly
- Use emojis (✨, 💪, 🎯, 📊)
- Keep responses concise (2-3 paragraphs)
- End with action item or encouragement

KEY INSIGHTS:
- Calculate exact numbers needed to catch up
- Suggest specific sales techniques
- Remind about Daily Combo (+100 pts)
- Recommend training based on gaps

You're their personal coach who cares about their success!"""
